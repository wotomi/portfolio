                  ...
                  >
                    <div className="flex gap-6 md:gap-8 group">
                      ...
                      <div className="relative flex-shrink-0">
                        <div className={`absolute inset-0 ${exp.iconBg} rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
                        <div className={`relative w-16 h-16 rounded-2xl ${exp.iconBg} border border-border flex items-center justify-center backdrop-blur-sm transition-all duration-300 group-hover:scale-110 group-hover:border-${exp.iconColor.split('-')[1]}-500/50`}>
                          <Icon className={`w-7 h-7 ${exp.iconColor}`} />
                        </div>
                        {/* Connecting dot */}
                        <div className="absolute left-1/2 -translate-x-1/2 -bottom-6 w-1 h-6 bg-gradient-to-b from-border to-transparent hidden md:block" />
                      </div>

                      {/* Content card */}
                      <div className="flex-1 pb-8">
                        <div className={`relative p-6 rounded-xl border border-border bg-background/50 backdrop-blur-sm overflow-hidden group-hover:border-${exp.iconColor.split('-')[1]}-500/30 transition-all duration-300`}>
                          {/* Gradient overlay */}
